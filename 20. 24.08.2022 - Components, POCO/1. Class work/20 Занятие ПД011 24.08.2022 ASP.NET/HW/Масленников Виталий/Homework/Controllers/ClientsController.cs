...
public class ClientsController : Controller
{
    ...
    // Конструктор
    public ClientsController(TravelCompany travelCompany, IWebHostEnvironment env, Services.Logger logger)
    {
    ...
    // Вывод сведений о клиентах по введенной фамилии
    ...
    public async Task<IActionResult> UpdateClientAsync(ClientInputForm inputModel)
    {
        ...
        // Если клиент (создаётся) или (изменяется и загружено изображение)
        ...
        sb.Append($"[{new DateTime().NowDateTimeString()}] ");
        sb.Append(index < 0 ? "Created: " : "Updated: ");
        sb.Append($"{inputModel.Client.Surname}, " +
                  $"{inputModel.Client.Name}, " +
                  $"{inputModel.Client.Patronymic}, " +
                  $"{inputModel.Client.Password}, " +
                  $"{inputModel.Client.Age}, " +
                  $"{inputModel.Client.Email}, " +
                  $"{inputModel.Client.PhoneNumber}, " +
                  $"Photo File: {inputModel.Client.Photo}, " +
                  $"Regular: {inputModel.Client.IsRegular} ");
