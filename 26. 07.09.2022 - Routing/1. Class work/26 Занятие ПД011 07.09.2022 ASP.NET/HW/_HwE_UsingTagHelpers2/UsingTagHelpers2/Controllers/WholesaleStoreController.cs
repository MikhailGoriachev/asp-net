...
public class WholesaleStoreController : Controller
{
    ...
    // вывод записей таблицы "Продавцы"
    ...
    // вывод записей таблицы "Продажи" с заданной сортировкой
    // и постраничной навигацией
    public async Task<IActionResult> Sales(int pageNumber = 1, SortState sortOrder = SortState.IdAsc) {
        ...
        // сохранить текущую сортировку во ViewBag, выполнить заданную сортировку
        ViewBag.SortOrder = sortOrder;  // режим сортировки правильнее сохранять в модели
        ViewBag.IdSort = sortOrder == SortState.IdAsc ? SortState.IdDesc : SortState.IdAsc;
        ViewBag.SaleDateSort = sortOrder == SortState.SaleDateAsc ? SortState.SaleDateDesc : SortState.SaleDateAsc;
        ViewBag.SalePriceSort = sortOrder == SortState.SalePriceAsc ? SortState.SalePriceDesc : SortState.SalePriceAsc;

        var orderedSales = sortOrder switch {
            SortState.IdDesc        => sales.OrderByDescending(s => s.Id),
            SortState.SaleDateAsc   => sales.OrderBy(s => s.DateSale),
            SortState.SaleDateDesc  => sales.OrderByDescending(s => s.DateSale),
            SortState.SalePriceAsc  => sales.OrderBy(s => s.PriceSale),
            SortState.SalePriceDesc => sales.OrderByDescending(s => s.PriceSale),
            _                       => sales.OrderBy(s => s.Id),
        };

        // получить набор записей для отображения на заданной странице
        var pageSales = orderedSales
            .Skip((pageNumber - 1) * ItemPageSize)
            .Take(ItemPageSize);

        return View(new PaginationPageViewModel<Sale>(
            await pageSales.ToListAsync(),
