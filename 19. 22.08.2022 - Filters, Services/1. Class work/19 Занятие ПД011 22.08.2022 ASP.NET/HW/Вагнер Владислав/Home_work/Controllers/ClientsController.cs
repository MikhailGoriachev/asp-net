...
public class ClientsController : Controller
{
    ...
    public ClientsController() {
        ...
        SortFuncts = new() {
            ["OrderBySnp"] = ("Сортировка по алфавиту",
                () => _clients?.OrderBy(c => c.Surname)
                               .ThenBy(c => c.Name)
    ...
    public IActionResult AddClient(int id) {
        ...
        //Создание объекиа
        return View("ClientsForm", new Client() { Id = maxId + 1 });
    }

    //Редактирование
    public IActionResult EditClient(int id) {
        Client? client = _clients.FirstOrDefault(b => b.Id == id);
        ...
        return View("ClientsForm", client);
    }

    //Post-обработчик формы на редактирование и добавление 
    [HttpPost]
    public IActionResult FormSubmit(Client model, bool addingmode, IFormFile? photoFile) {
    ...
    public IActionResult DeleteClient(int id) {
        Client? deletingClient = _clients.FirstOrDefault(b => b.Id == id);
        ...
        //Фамилии для выпадающего списка
        ViewBag.SelectSurnames = _surnamesSelect;
        ViewBag.Header = $"Удалён клиент с Id: {id}";
